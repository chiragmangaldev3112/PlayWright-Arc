...
test('@high @ui @smoke Get user by ID', async ({ request }) => {
  ...
  const response = await api.getUser({ data: undefined, params: {
  "id": 1
...
test('@critical @ui @e2e Create a new user', async ({ request }) => {
  ...
  "username": "string",
  "email": "string"
} as Models.CreateUserRequest, params: undefined });
  const data = await response.json();
  console.log(data);
});
...
test('@high @ui @regression Create a new user - Create user with missing email', async ({ request }) => {
  ...
  const response = await api.createUser({ data: {
  "username": "string"
} as Models.CreateUserValidation1Request, params: undefined });
...
test('@high @ui @regression Update existing user', async ({ request }) => {
  ...
  "username": "string",
  "email": "string"
} as Models.UpdateUserRequest, params: {
  "id": 1
...
test('@medium @ui @regression Update existing user - Update user with invalid ID', async ({ request }) => {
  ...
  "username": "string",
  "email": "string"
} as Models.UpdateUserValidation1Request, params: {
  "id": 99999
...
test('@medium @ui @regression Delete user by ID', async ({ request }) => {
  const api = new UserApi(request);
  const response = await api.deleteUser({ data: undefined, params: {
  "id": 1
